	...
	public static Response process(Request request) throws Exception {
		try {
			...
			if (request instanceof GetProcessIdentifiersRequest) {
				...
				LearningResult result = Learning.learn(lRequest.getDesign(), lRequest.getEvaluationResult(), lRequest.getSelectedOutputIdentifier(), lRequest.getCovarianceFunction(), lRequest.getLengthScale(), lRequest.getProcessVariance(), lRequest.getNuggetVariance(), lRequest.getMeanFunction(), lRequest.isNormalisation());
