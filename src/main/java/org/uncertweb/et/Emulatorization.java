	...
	public static Response process(Request request) throws Exception {
		try {
			...
			if (request instanceof GetProcessIdentifiersRequest) {
				...
				LearningResult result = Learning.learn(lRequest.getTrainingDesign(), lRequest.getTrainingEvaluationResult(), lRequest.getSelectedOutputIdentifier(), lRequest.getCovarianceFunction(), lRequest.getLengthScale(), lRequest.getProcessVariance(), lRequest.getNuggetVariance(), lRequest.getMeanFunction(), lRequest.isNormalisation());
				...
				try {
					c = R.getConnection();
					rserveOK = true;
					rserveMessage = c.eval("R.Version()$version.string").asString() + " ready for action!";
				}
